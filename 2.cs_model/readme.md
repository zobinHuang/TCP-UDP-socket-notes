...
There might be some confusions of this **new socket**. Let's make it more clear. You should know that the **Server** process needs to serve multiple **Client** eventually. So how? The answer is, we use the **main socket** to receive TCP connection requests (aka *SYN*). Once there is a connection request, we copy a **new socket** and use it to complete the connection establishment and subsequent data transmission. In this way, the **Server** can interact with multiple **Client** via multiple so called **new socket**s. \
